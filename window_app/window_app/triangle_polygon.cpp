...
[[nodiscard]] bool TrianglePolygon::createVertexBuffer() noexcept
{
	...
	//頂点バッファの生成
	auto res = device::instance().get()->CreateCommittedResource(&heapProperty, D3D12_HEAP_FLAG_NONE, &resourceDesc, D3D12_RESOURCE_STATE_GENERIC_READ, nullptr, IID_PPV_ARGS(&vertexBuffer_));
	...
	//バッファをマップ(CPU からアクセス可能にする)
	//vertexBuffer_ 直接利用するのではなく、 data を介して更新するイメージ
	...
	//コピーが終わったのでマップ解除(CPU アクセス不可にする)
	//ここまで来たらGPU が利用するメモリ(VRAM) にコピー済みなので、triangleVertices は不要になる
...
[[nodiscard]] bool TrianglePolygon::createIndexBuffer() noexcept
{
	...
	//インデックスバッファの生成
	auto res = device::instance().get()->CreateCommittedResource(&heapProperty, D3D12_HEAP_FLAG_NONE, &resourceDesc, D3D12_RESOURCE_STATE_GENERIC_READ, nullptr, IID_PPV_ARGS(&indexBuffer_));

	if (FAILED(res))
	{
		assert(false && "インデックスバッファの生成に失敗");
		return false;
	}
	...
	//ここまで来たらGPU が利用するメモリ領域にコピー済みなので、triangledicesは不要になる

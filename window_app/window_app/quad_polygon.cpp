...
[[nodiscard]] bool QuadPolygon::createVertexBuffer() noexcept
{
	...
	//コピーが終わったのでマップ解除(CPU からアクセス不可にする)
...
[[nodiscard]] bool QuadPolygon::createIndexBuffer() noexcept
{
	...
	//ここまで来たらGPU が利用するメモリ(VRAM) にコピー済みなので、indicesは不要になる
	indexBuffer_->Unmap(0, nullptr);

	//インデックスバッファビュー作成
	indexBufferView_.BufferLocation = indexBuffer_->GetGPUVirtualAddress();
	indexBufferView_.SizeInBytes = indexBufferSize;
	indexBufferView_.Format = DXGI_FORMAT_R16_UINT;	//indices の型が16bit 符号なしの整数なのでR16_UINT

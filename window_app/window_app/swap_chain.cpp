...
[[nodiscard]] bool SwapChain::create(const CommandQueue& commandQueue) noexcept
{
	...
	swapChainDesc_.SwapEffect			= DXGI_SWAP_EFFECT_FLIP_DISCARD;	//毎フレーム画面更新するので描写が終わったらバッファを破棄
	...
	{
		const auto hr = device::instance().dxgi().factory()->CreateSwapChainForHwnd(commandQueue.get(),window::instance().handle(), &swapChainDesc_, nullptr, nullptr, tempSwapChain.GetAddressOf());
